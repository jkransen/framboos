...
public class GpioPin {
	// REV 1:
	// private static final int [] mappedPins = {17, 18, 21, 22, 23, 24, 25, 4,
	// 0, 1, 8, 7, 10, 9, 11, 14, 15};
	// REV 2:
	private static final int[] mappedPins = { 17, 18, 27, 22, 23, 24, 25, 4, 2,
			3, 8, 7, 10, 9, 11, 14, 15 };
	public static Pattern pinPattern = Pattern.compile(
			"(gpio)([0-9])_([0-9]*)", Pattern.CASE_INSENSITIVE);
	public static Pattern pinPatternAlt = Pattern.compile("(gpio)([0-9]*)",
	...
	public GpioPin(int pinNumber, Direction direction, boolean direct) {
		/*
		 * Allow Override of the mapping for people to directly drive the GPIOs
		 * in the numbering they want
		 */
		if (!direct) {
			this.pinNumber = mappedPins[pinNumber];
		} else {
	...
	public boolean getValue() {
		...
		try {
			...
			throw new RuntimeException("Could not read from GPIO file: "
					+ e.getMessage());
		}
	}
